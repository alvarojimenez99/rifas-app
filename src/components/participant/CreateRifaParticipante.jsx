...
// Mapa completo de colores (igual que el original)
const coloresMap = {
...
const CreateRifaParticipante = () => {
  ...
  const tiposRifas = {
    numeros: { descripcion: 'Números del 1 al N' },
    baraja: { descripcion: 'Cartas de la baraja española' },
    abecedario: { descripcion: 'Letras de la A a la Z' },
    animales: { descripcion: 'Animales del zodiaco chino' },
    colores: { descripcion: 'Colores básicos' },
    equipos: { descripcion: 'Equipos de fútbol' },
    emojis: { descripcion: 'Emojis divertidos' },
    paises: { descripcion: 'Países del mundo' }
  };
  ...
  const manejarCambioTipo = (tipo) => {
    const nuevosElementos = generarElementosRifa(tipo, nuevaRifa.cantidadNumeros);
    setNuevaRifa({
      ...nuevaRifa,
      tipo: tipo,
  ...
  const manejarCrearRifa = async () => {
    ...
    const datosParaEnviar = {
      ...nuevaRifa,
      cantidadElementos: nuevaRifa.cantidadNumeros, // 🔧 CORREGIDO
    ...
    // Eliminar el campo cantidadNumeros para no confundir al backend
    delete datosParaEnviar.cantidadNumeros;
    ...
    try {
      ...
      if (response.ok && data.success) {
        ...
        showError('Erro', data.error || 'Error al crear la rifa');
      }
    } catch (error) {
      console.error('Error:', error);
      showError('Erro', 'Error al crear la rifa');
    } finally {
      setEnviando(false);
    }
  };
                  ...
                  <div className="fotos-grid-modern">
                    {nuevaRifa.fotosPremios.map((foto, idx) => (
                      <div key={foto.id || idx} className="foto-item-modern">
                        ...
                        <button type="button" className="btn-remover-foto" onClick={() => manejarEliminarFoto(foto.id)}>✕</button>
                      </div>
                    ))}
                  </div>
                )}
  ...
  return (
    <div className="create-rifa-wizard-modern">
      {/* Banner de información para participante */}
      <div className="participante-banner">
        <div className="banner-icon">⏳</div>
        <div className="banner-content">
          <h3>Sua rifa será analisada pelo administrador</h3>
          <p>
            Após criar a rifa, ela ficará com status <strong>"Pendente de Aprovação"</strong>.
            O administrador irá revisar e aprovar. Você será notificado quando for aprovada!
          </p>
        </div>
      </div>
      ...
      <div className="wizard-header-modern">
        <div className="header-content">
          ...
          <p className="wizard-subtitle">Preencha os dados abaixo para solicitar sua rifa</p>
        </div>
      ...
      <div className="wizard-actions-modern">
        ...
        {pasoActual < totalPasos ? (
          ...
          <div className="final-step-actions">
            <button type="button" className="btn-wizard-create" onClick={manejarCrearRifa} disabled={!puedeContinuar() || subiendoFotos || enviando}>
              <span className="btn-icon">📨</span>
              <span>{enviando ? 'Enviando...' : 'Solicitar Rifa'}</span>
            </button>
          </div>
        )}
      </div>
      ...
      {mostrarTerminos && (
        <TermsAndConditions onAccept={manejarAceptarTerminos} onDecline={() => setMostrarTerminos(false)} />
      )}
      ...
      {mostrarMensajeExito && (
        <div className="modal-overlay">
          <div className="modal-content success-modal">
            ...
            <p>Sua rifa foi enviada para aprovação do administrador.</p>
            <p className="status-info">Status atual: <strong>⏳ Pendente de Aprovação</strong></p>
            <p>Você será notificado quando for aprovada.</p>
            <div className="success-actions">
              <button className="btn-primary" onClick={() => navigate('/dashboard-participante')}>Ver minhas rifas</button>
              <button className="btn-secondary" onClick={() => navigate('/')}>Ir para o Início</button>
            </div>
          </div>
        </div>
      )}
...
export default CreateRifaParticipante;